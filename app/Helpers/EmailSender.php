    ...
    public function sendEmail($registration, $subject, $template) {
        try {
            ...
            $params = [
                ...
                'event' => $event->name,
                'rpc_start_date' => $event->rpc_start_date ? Carbon::parse($event->rpc_start_date)->format('d M Y H:i') : '-',
                'rpc_end_date' => $event->rpc_end_date ? Carbon::parse($event->rpc_end_date)->format('d M Y H:i') : '-',
